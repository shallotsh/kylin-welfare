...
public class ContainFourProcessor implements SequenceProcessor {
    ...
    public List<WCode> process(List<WCode> deletedCodes) {
        ...
        for(WCode wCode: wCodes){
            if(WCodeUtils.containInSet(wCode, boldCodes) >= 4){
                ret.add(wCode);
...
//        wCodes.stream().filter(wCode -> WCodeUtils.containInSet(wCode, boldCodes) >= 4).collect(Collectors.toList());
