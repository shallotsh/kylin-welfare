...
public class OrderedFishManProcessor implements SequenceProcessor {
    ...
    private List<List<Integer>> boldCodes;
    ...
    public SequenceProcessor init(WCodeReq wCodeReq) {
        if(wCodeReq != null){
           ...
           boldCodes = TransferUtil.parseLists(wCodeReq.getBoldCodeFive());
        }
        return this;
    }
    ...
    public List<WCode> process(List<WCode> deletedCodes) {
        ...
        List<WCode> ret = wCodes.stream().filter(wCode -> WCodeUtils.isInFishCodeWithOrder(wCode, boldCodes)).collect(Collectors.toList());
